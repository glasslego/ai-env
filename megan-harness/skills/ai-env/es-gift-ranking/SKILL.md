---
name: es-gift-ranking
description: 선물하기 랭킹 특화 Elasticsearch 쿼리 스킬. Execute queries against production(prod) or stage environments.
...
_SKILL_DIR = Path(__file__).parent if '__file__' in dir() else Path.home() / ".claude/skills/es-gift-ranking"
with open(_SKILL_DIR / '.config.json') as f:
    config = json.load(f)
