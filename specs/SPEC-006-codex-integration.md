...
| `src/ai_env/core/codex_skills.py` | Codex YAML frontmatter 정규화 |
| `src/ai_env/core/project_sync.py` | 프로젝트 로컬 Claude→Codex 동기화 |
| `tests/test_ai_env.py` | Codex 설정 생성 테스트 (`TestGenerateCodexConfig`) |
| `tests/core/test_codex_skills.py` | Codex 스킬 정규화 테스트 |

## 13. Codex Skills YAML Frontmatter 정규화

### 13.1 배경

Codex CLI(0.113.0+)는 SKILL.md의 YAML frontmatter를 엄격하게 파싱한다.
Claude Code에서는 허용되는 느슨한 형식(multi-line description, 비표준 들여쓰기)이
Codex에서는 파싱 에러를 일으킨다.

### 13.2 정규화 로직 (`codex_skills.py`)

`normalize_skill_markdown_for_codex()` 함수가 느슨한 frontmatter를 strict YAML로 변환:

1. `---` 구분자 안의 frontmatter에서 `name`, `description` 추출
2. `yaml.safe_dump()`으로 strict YAML 재생성
3. body 내용은 그대로 유지
...
---
name: my-skill
description: "여러 줄의 설명\nUse when: ..."
---
```

### 13.3 적용 지점

- **글로벌 sync**: `sync.py`의 `_sync_codex_skills_merged()` → `copy_skill_tree_for_codex()`
- **프로젝트 sync**: `project_sync.py`의 `_sync_codex_skills()` → `copy_skill_tree_for_codex()`
- 두 경로 모두 `safe_copytree()` → SKILL.md 순회 → 정규화 순서로 동작
