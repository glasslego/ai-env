...
├─ Claude Code      (settings.json, CLAUDE.md, commands/, skills/, hooks/)
├─ Codex CLI Global (config.toml, AGENTS.md, skills/)
├─ Codex CLI Local  (config.toml, skills/)
...
│   │   ├── secrets.py         # SecretsManager (regex 기반 ${VAR} 치환)
│   │   ├── sync.py            # Claude/Codex/Gemini 글로벌 설정 동기화
│   │   ├── project_sync.py    # 프로젝트 로컬 Claude↔Codex 동기화
│   │   ├── codex_skills.py    # Codex YAML frontmatter 정규화
│   │   ├── doctor.py          # 환경 건강 검사
│   │   ├── pipeline.py        # 리서치 파이프라인 유틸
│   │   ├── research.py        # Deep Research API 디스패치
│   │   └── workflow.py        # 6-Phase 워크플로우 관리
│   ├── mcp/
│   │   ├── __init__.py        # MCPConfigGenerator re-export
│   │   ├── generator.py       # 타겟별 MCP 설정 생성
│   │   └── vibe.py            # Agent Fallback 쉘 함수 생성
│   └── cli/                   # Click CLI + Rich UI (모듈 분리)
├── .claude/
│   ├── global/                # CLAUDE.md, settings.json.template
│   ├── commands/              # 슬래시 커맨드 (.md 파일)
│   ├── skills/                # 개인 스킬 (SKILL.md 기반)
│   ├── hooks/                 # 세션 lifecycle 훅 (session_start/end, pre_compact)
│   ├── handoff/               # 세션 간 컨텍스트 전달 (gitignore)
│   ├── logs/                  # fallback 세션 로그 (gitignore)
...
**사용 예**: GitHub MCP 서버는 `GITHUB_PERSONAL_ACCESS_TOKEN`을 기대하지만, `.env`에는 용도별로 `GITHUB_GLASSLEGO_TOKEN`, `GITHUB_TOKEN`으로 구분 저장한다. 생성 시 키 이름을 변환하여 Docker `-e` 옵션에 올바른 이름으로 전달한다.
