...
def _run_bash_with_pty(command: str, env: dict[str, str], timeout: float = 30.0) -> tuple[int, str]:
    """Run a bash command under a PTY so wrapper code sees an interactive TTY."""
...
def _prepare_fake_sync_env(home_dir: Path, bin_dir: Path) -> None:
    """Create the minimal ai-env tree that the generated shell wrapper expects."""

    ai_env_dir = home_dir / "work" / "glasslego" / "ai-env"
    (ai_env_dir / "cde-skills" / ".git").mkdir(parents=True, exist_ok=True)
...
class TestGenerateShellFunctions:
    ...
    def test_contains_tty_aware_sync_modes(self):
        """스킬 sync가 TTY 여부에 따라 foreground/background 모드를 나누는지 확인."""
        gen = self._make_generator(["claude", "codex"])
        result = gen.generate_shell_functions()

        assert "_ai_env_sync_skills_run()" in result
        assert 'local _mode="${1:-auto}"' in result
    ...
    def test_interactive_sync_finishes_before_claude_starts(self, tmp_path):
        ...
        returncode, output = _run_bash_with_pty(
            f'rm -f /tmp/.ai_env_skills_sync.lock && source "{fn_file}" && claude --fallback hello',
            env,
        )

        assert returncode == 0, output
        assert "SYNC-LIVE" in output
        lines = trace_file.read_text().splitlines()
        claude_idx = next(i for i, line in enumerate(lines) if line.startswith("claude:"))
        assert "uv-end" in lines[:claude_idx]

    def test_interactive_sync_finishes_before_codex_starts(self, tmp_path):
        """codex wrapper도 인터랙티브 TTY에서는 sync 후에 시작되어야 한다."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        ...
        returncode, output = _run_bash_with_pty(
            f'rm -f /tmp/.ai_env_skills_sync.lock && source "{fn_file}" && codex run-task',
            env,
        )

        assert returncode == 0, output
        lines = trace_file.read_text().splitlines()
        codex_idx = next(i for i, line in enumerate(lines) if line.startswith("codex:"))
        assert "uv-end" in lines[:codex_idx]

    def test_non_tty_sync_output_stays_silent(self, tmp_path):
        """비대화형 실행에서는 background sync 출력이 명령 출력에 섞이지 않아야 한다."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        ...
        assert result.returncode == 0, result.stdout + result.stderr
        assert "CODEX-LIVE" in result.stdout
        assert "SYNC-LIVE" not in result.stdout + result.stderr
...
class TestSessionLogAndExit:
    ...
    def test_session_log_saved_to_log_dir(self, tmp_path):
        ...
        # {session_id}_claude.log 파일 존재 확인
        log_files = list(log_dir.glob("*_claude.log"))
    ...
    def test_session_id_in_filenames(self, tmp_path):
        """파일명이 {session_id}_{agent}.log 형식."""
        gen = self._make_generator(["claude"])
        shell_fn = gen.generate_shell_functions()
        ...
        # 디렉토리명 prefix 없이 {session_id}_{agent}.log 형식 (내부 메타파일 제외)
        all_files = [f for f in log_dir.glob("*") if not f.name.startswith(".")]
        assert len(all_files) >= 1
        for f in all_files:
            assert "_claude" in f.name
            assert "__" not in f.name  # 더 이상 dir_prefix__ 패턴 사용 안 함
...
class TestCodexTransitionEdgeCases:
    """Codex 전환 엣지 케이스: 스무스하지 않은 전환 시나리오 검증"""
    ...
    def _setup_env(self, tmp_path):
        ...
        env["TRACE_FILE"] = str(trace_file)
        env.pop("CLAUDECODE", None)
        return bin_dir, trace_file, fn_file, env

    def test_claude_crash_no_rate_limit_passes_original_to_codex(self, tmp_path):
        """Claude가 rate-limit 아닌 일반 에러로 종료 → codex에 원래 프롬프트 전달."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        bin_dir, trace_file, fn_file, env = self._setup_env(tmp_path)

        # Claude: 에러 메시지 출력 후 exit 1 (rate-limit 아님)
        claude_script = bin_dir / "claude"
        claude_script.write_text(
            "#!/usr/bin/env bash\n"
            'echo "claude" >> "$TRACE_FILE"\n'
        ...
        result = subprocess.run(
            ["bash", "-c", f"source {fn_file} && claude --fallback 'build feature'"],
            env=env,
            text=True,
            capture_output=True,
            check=False,
        ...
        lines = trace_file.read_text().splitlines()
        assert lines[0] == "claude"
        # rate-limit 아니므로 handoff 없이 원래 프롬프트가 codex exec로 전달
        codex_line = lines[1]
        assert codex_line.startswith("codex:")
        assert "exec" in codex_line
        assert "build feature" in codex_line
        # handoff 키워드가 없어야 함 (rate-limit이 아니므로 핸드오프 안 생김)
        assert "handoff" not in codex_line.lower()

    def test_codex_exec_exact_args_structure(self, tmp_path):
        """Codex exec 모드에서 정확한 인자 구조 검증."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        bin_dir, trace_file, fn_file, env = self._setup_env(tmp_path)
        ...
        # Codex: 각 인자를 개별적으로 기록
        codex_script = bin_dir / "codex"
        codex_script.write_text(
            "#!/usr/bin/env bash\n"
            'for arg in "$@"; do echo "arg:$arg" >> "$TRACE_FILE"; done\n'
            "exit 1\n"
        )
        codex_script.chmod(codex_script.stat().st_mode | stat.S_IXUSR)
        ...
        subprocess.run(
            ["bash", "-c", f"source {fn_file} && claude --fallback 'my task'"],
            env=env,
            text=True,
            capture_output=True,
            check=False,
        ...
        assert arg_lines[0] == "arg:exec"
        assert arg_lines[1] == "arg:-c"
        assert arg_lines[2] == "arg:approval_policy='never'"
        assert arg_lines[3] == "arg:-s"
        assert arg_lines[4] == "arg:workspace-write"
        # 마지막 인자는 핸드오프 프롬프트 (원래 작업 포함)
        assert "my task" in arg_lines[5]

    def test_claude_crash_no_prompt_codex_gets_yolo(self, tmp_path):
        """Claude가 프롬프트 없이 비정상 종료 시 codex가 --yolo 모드로 시작."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        bin_dir, trace_file, fn_file, env = self._setup_env(tmp_path)
        ...
        lines = trace_file.read_text().splitlines()
        assert lines[0] == "claude"
        # 프롬프트 없이 crash → codex는 --yolo 모드
        assert "codex:--yolo --no-alt-screen" == lines[1]

    def test_fallback_with_special_chars_in_prompt(self, tmp_path):
        """프롬프트에 특수문자(인용부호, 백틱 등)가 있어도 codex에 정상 전달."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        bin_dir, trace_file, fn_file, env = self._setup_env(tmp_path)
        ...
        # 특수문자가 포함된 프롬프트
        prompt = "fix the bug in user's config.py"
        result = subprocess.run(
            ["bash", "-c", f'source {fn_file} && claude --fallback "{prompt}"'],
            env=env,
            text=True,
            capture_output=True,
            check=False,
        ...
        lines = trace_file.read_text().splitlines()
        assert lines[0] == "claude"
        # 프롬프트가 codex에 전달되었는지 확인
        assert "config.py" in lines[1]

    def test_only_codex_available_claude_not_installed(self, tmp_path):
        """Claude가 설치되지 않은 환경에서 codex만 실행."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        bin_dir, trace_file, fn_file, env = self._setup_env(tmp_path)

        # Claude 바이너리 없음 (bin_dir에 codex만 생성)
        codex_script = bin_dir / "codex"
        codex_script.write_text('#!/usr/bin/env bash\necho "codex:$*" >> "$TRACE_FILE"\nexit 0\n')
        codex_script.chmod(codex_script.stat().st_mode | stat.S_IXUSR)

        fn_file.write_text(shell_fn)
        # PATH를 bin_dir + 최소 시스템 경로로 제한하여 시스템 claude를 숨김
        ...
        assert result.returncode == 0
        lines = trace_file.read_text().splitlines()
        # Claude 건너뛰고 codex만 실행
        assert any("codex:" in line for line in lines)
        # "설치되지 않음" 메시지 출력
        combined = result.stdout + result.stderr
        assert "설치되지 않음" in combined or "건너뜀" in combined

    def test_codex_nonzero_exit_shows_error_and_terminates(self, tmp_path):
        """Codex가 마지막 에이전트로 비정상 종료 시 에러 메시지 출력 후 종료."""
        gen = self._make_generator(["codex"])
        shell_fn = gen.generate_shell_functions()
        bin_dir, trace_file, fn_file, env = self._setup_env(tmp_path)
        ...
        assert result.returncode != 0
        combined = result.stdout + result.stderr
        assert "소진" in combined or "에이전트" in combined

    def test_rapid_rate_limit_no_delay_before_codex(self, tmp_path):
        """Claude가 즉시 rate-limit → codex 전환에 불필요한 지연 없음."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        bin_dir, trace_file, fn_file, env = self._setup_env(tmp_path)

        # Claude: 즉시 rate-limit
        claude_script = bin_dir / "claude"
        claude_script.write_text(
            "#!/usr/bin/env bash\n"
            'echo "claude:$(date +%s)" >> "$TRACE_FILE"\n'
            'echo "rate limit exceeded"\n'
            "exit 1\n"
        )
        claude_script.chmod(claude_script.stat().st_mode | stat.S_IXUSR)
        ...
        lines = trace_file.read_text().splitlines()
        assert lines[0].startswith("claude:")
        assert any(line.startswith("codex:") for line in lines)

        # Claude → Codex 전환이 10초 이내에 완료 (script PTY 오버헤드 포함)
        assert elapsed < 10, f"전환에 {elapsed:.1f}초 소요 — 5초 이내여야 함"

    def test_complete_opus_sonnet_codex_flow_with_exec(self, tmp_path):
        """전체 경로: opus RL → sonnet RL → codex exec 성공, 각 단계 인자 검증."""
        gen = self._make_generator(["claude", "claude:sonnet", "codex"])
        shell_fn = gen.generate_shell_functions()
        bin_dir, trace_file, fn_file, env = self._setup_env(tmp_path)
        ...
        subprocess.run(
            ["bash", "-c", f"source {fn_file} && claude --fallback 'deploy service'"],
            env=env,
            text=True,
            capture_output=True,
            check=False,
        ...
        # 1단계: opus (no --model flag)
        assert lines[0] == "claude:deploy service"
        # 2단계: sonnet (--model sonnet flag)
        assert lines[1].startswith("claude:--model sonnet")
        # 3단계: codex exec (핸드오프 포함)
        codex_line = lines[2]
        assert codex_line.startswith("codex:")
        assert "exec" in codex_line
        assert "deploy service" in codex_line


class TestRateLimitPatternCoverage:
    """다양한 Claude rate-limit 메시지 패턴 감지 검증."""
    ...
    def _run_with_rate_limit_message(
        self, tmp_path, message: str, exit_code: int = 0
    ) -> tuple[list[str], str]:
        """Claude가 특정 rate-limit 메시지를 출력하고, fallback이 트리거되는지 확인."""
        gen = self._make_generator(["claude", "codex"])
        shell_fn = gen.generate_shell_functions()
        ...
        return lines, result.stdout + result.stderr

    def test_switch_to_extra_usage(self, tmp_path):
        """'switch to extra usage' 패턴으로 fallback 트리거."""
        lines, _ = self._run_with_rate_limit_message(tmp_path, "switch to extra usage", exit_code=1)
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_upgrade_your_plan(self, tmp_path):
        """'upgrade your plan' 패턴으로 fallback 트리거."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "upgrade your plan to continue", exit_code=1
        )
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_stop_and_wait_for_limit(self, tmp_path):
        """'stop and wait for limit to reset' 패턴으로 fallback 트리거."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "stop and wait for limit to reset", exit_code=1
        )
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_you_have_exhausted(self, tmp_path):
        """'you have exhausted' 패턴으로 fallback 트리거."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "you have exhausted your daily quota", exit_code=1
        )
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_too_many_requests(self, tmp_path):
        """'too many requests' 패턴으로 fallback 트리거 (exit 1)."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "Error: too many requests", exit_code=1
        )
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_rate_limit_options_with_exit_zero(self, tmp_path):
        """'/rate-limit-options' UI 요소가 exit 0에서도 감지."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "/rate-limit-options  /reset-rate-limit", exit_code=0
        )
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_what_do_you_want_to_do(self, tmp_path):
        """'what do you want to do' 패턴 (rate-limit 화면 후 선택지)."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "what do you want to do?", exit_code=1
        )
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_reached_daily_limit(self, tmp_path):
        """'reached your daily limit' 패턴."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "You have reached your daily limit", exit_code=1
        )
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_exceeded_hourly_limit(self, tmp_path):
        """'exceeded hourly limit' 패턴."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "exceeded hourly limit for this model", exit_code=1
        )
        assert lines[0] == "claude"
        assert "codex" in lines
    ...
    def test_quota_exceeded(self, tmp_path):
        """'quota exceeded' 패턴."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "API quota exceeded. Please try again later.", exit_code=1
        )
        assert lines[0] == "claude"
        assert "codex" in lines

    def test_no_false_positive_on_normal_output(self, tmp_path):
        """일반 출력에서는 rate-limit 오탐 없음."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "I fixed the rate calculation in the code", exit_code=0
        )
        assert lines == ["claude"]  # codex 호출 없음

    def test_no_false_positive_on_limit_in_code(self, tmp_path):
        """코드 관련 'limit' 출력에서 오탐 없음."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "Added rate_limit parameter to the API endpoint", exit_code=0
        )
        assert lines == ["claude"]  # codex 호출 없음

    def test_hit_your_limit_resets_with_timezone(self, tmp_path):
        """타임존 포함 리셋 메시지 감지."""
        lines, _ = self._run_with_rate_limit_message(
            tmp_path, "You've hit your limit · resets 2pm (Asia/Seoul)", exit_code=0
        )
        assert lines[0] == "claude"
        assert "codex" in lines


class TestHandoffFileIntegrity:
    """핸드오프 파일 내용 무결성 검증."""
    ...
    def test_handoff_file_contains_required_sections(self, tmp_path):
        """핸드오프 파일에 필수 섹션(Original Task, Instructions)이 포함."""
        log_dir = tmp_path / "log"
        gen = self._make_generator(["claude", "codex"], fallback_log_dir=str(log_dir))
        shell_fn = gen.generate_shell_functions()
        ...
        subprocess.run(
            ["bash", "-c", f"source {fn_file} && claude --fallback 'fix login bug'"],
            env=env,
            text=True,
            capture_output=True,
            check=False,
        ...
        assert "이어서 진행" in content

    def test_handoff_original_task_empty_for_interactive(self, tmp_path):
        """대화형 세션의 핸드오프에 '대화형 세션' 표시."""
        log_dir = tmp_path / "log"
        gen = self._make_generator(["claude", "codex"], fallback_log_dir=str(log_dir))
        shell_fn = gen.generate_shell_functions()
