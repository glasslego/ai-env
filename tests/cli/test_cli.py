...
def test_project_sync_codex_command(runner, tmp_path):
    """Test project sync-codex command."""
    project_dir = tmp_path / "sample-project"
    project_dir.mkdir()

    with patch("ai_env.cli.project_cmd.sync_project_claude_to_codex") as mock_sync:
        mock_sync.return_value = []
        result = runner.invoke(main, ["project", "sync-codex", "--project-dir", str(project_dir)])

    assert result.exit_code == 0, f"Command failed with output: {result.output}"
    mock_sync.assert_called_once()
    assert "Project Claude" in result.output
