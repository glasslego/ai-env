...
모든 AI 코딩 에이전트(Claude, Codex)는 아래 순서를 반드시 따른다.

1) **Spec 기준으로 Task를 식별**
- 구현 시작 전에 어떤 Spec/Task를 수행하는지 명확히 기록한다.
- Spec이 없거나 Task 경계가 모호하면 구현 전에 사용자에게 확인한다.

2) **Task 단위로만 구현**
- 한 번에 하나의 Task만 구현한다.
- 요청 범위를 넘는 구조 변경/리팩터링은 금지한다.

3) **테스트 통과 후 커밋**
- Task 구현 후 관련 테스트를 먼저 실행한다.
- 테스트 실패 상태에서는 커밋하지 않는다.
- 테스트 수정은 "실패 원인 해결" 목적일 때만 허용하며, 통과를 위한 완화는 금지한다.
