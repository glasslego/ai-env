...
**CRUD**: `read`, `create`, `update`, `delete` (모두 `--page-id` 또는 `--url` 사용)
**검색**: `cql`, `search`, `children`, `ancestors`, `parse-url`
**메타**: `labels`, `add-label`, `remove-label`, `history`, `version`, `watchers`
**파일**: `attachments`, `upload`, `download`
**기타**: `comments`, `add-comment`, `copy`, `move`, `space`

전체 옵션은 `uv run python $CLI --help` 또는 `uv run python $CLI <command> --help` 참조.
