...
│   │
│   ├── doc-sync/                   #   예: 문서-코드 정합성 동기화
│   ├── spark-debug/                #   예: Spark 디버깅 스킬
│   ├── elasticsearch-query/        #   예: ES 쿼리 빌더 스킬
│   ├── pyspark-best-practices/     #   예: PySpark 코딩 가이드
│   └── ...
│
├── hooks/                          # [사용자 정의] 세션 lifecycle 훅
│   ├── session_start.sh            #   세션 시작 시: 이전 handoff 로드 + 팀 스킬 pull
│   ├── session_end.sh              #   세션 종료 시: handoff 저장 + git 상태 기록
│   └── pre_compact.sh              #   compact 전: transcript 백업 + handoff fallback
│
├── handoff/                        # [자동/수동] 세션 간 컨텍스트 전달 (gitignore)
│   ├── latest.md                   #   현재 handoff (session_start에서 로드)
│   └── archive/                    #   이전 handoff (최근 10개 보관)
│
...
```
ai-env repo 구조:
  .claude/global/settings.json.template            ← team 기본 프로필
  .claude/global/settings.personal.json.template   ← 개인 프로필

동기화 후:
  ~/.claude/settings.json                         ← team 기본 프로필 (팀 플랜 OAuth 로그인)
  ~/.claude/settings.enterprise.json              ← legacy alias용 동일 설정
  ~/.claude-personal/settings.json                ← 개인 프로필 (개인 로그인)
...
- `claude`/`claude team`은 `~/.claude`(팀 플랜 OAuth), `claude personal`은
  `CLAUDE_CONFIG_DIR=~/.claude-personal`(개인 Anthropic 로그인)로 실행
- `--settings`는 User 계층을 대체하지 못하므로 프로필 격리는 `CLAUDE_CONFIG_DIR`로만 가능하다
